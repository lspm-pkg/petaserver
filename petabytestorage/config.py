...
class Cache:
    ...
    COMPRESSION_LEVEL: int = config.get("cache", {}).get("compression_level", 3)
...
if len(ENCRYPTION_KEY) != 32:
    raise RuntimeError(f"ENCRYPTION_KEY must be 32 bytes encoded in hex, not {len(ENCRYPTION_KEY)} bytes")
